...
namespace DashBoard.Service.BackgroundServices
{
    public class TicketSyncBackgroundService : BackgroundService
    {
        ...
        public TicketSyncBackgroundService(
            IServiceScopeFactory scopeFactory,
            ILogger<TicketSyncBackgroundService> logger)
        {
                ...
                try
                {
                    ...
                    var glpiService =
                        scope.ServiceProvider
                            .GetRequiredService<IGLPIService>();

                    _logger.LogInformation(
                        "Starting GLPI ticket synchronization...");

                    await glpiService.SyncTicketsAsync();

                    _logger.LogInformation(
                        "GLPI ticket synchronization completed.");
                }
                catch (Exception ex)
                {
                    _logger.LogError(
                        ex,
                        "Error while synchronizing GLPI tickets.");
                }
